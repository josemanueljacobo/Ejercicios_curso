...
-- 22. Realiza una consulta para obtener todos los clientes y sus pedidos correspondientes utilizando un inner join.
SELECT Clientes.id AS cliente_id, c.nombre, p.id AS pedido_id
FROM Clientes c
LEFT JOIN Pedidos ON c.id = pcliente_id;
...
SELECT pr.id AS producto_id, pr.nombre, dp.pedido_id
FROM Productos pr
